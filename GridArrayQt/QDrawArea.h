...
public:
	...
	void setCursorR(short unsigned int r)				//set brush and erase cursor size
		{nCursorR = r;}
	void setKeySpace(bool k)							//set the space key status.
		{bKeySpaceEn = k;}
	void setPointCursor(QPointCursor pc,QCursor &c)
		{qPointCursor = pc;this->setCursor(c);}			//set point cursor status.
	void setCursorGray(int g);							//set grid line color(gray).
	...
	void loadPicture(QString&,byte,int);
	void unloadPicture();
...
private:
	//Key status
	bool bKeySpaceEn;									//Key space is pressed.
	bool bMouseMoving;									//Mouse is moving and button is pressed.
