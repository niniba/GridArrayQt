...
void QDrawArea::setCursorGray(int g)
{
	nGridGray = (byte)g;
	drawGrid();
}
...
void QDrawArea::loadPicture(QString &str,byte type,int value)
{
	if(bLoadedPic)
		delete qLoadImg;
	bLoadedPic = true;
...
void QDrawArea::mousePressEvent(QMouseEvent *e)
{
	if(e->button() & Qt::LeftButton)
	{
		thisPoint = e->pos();
		lastPoint = thisPoint;
		bMouseMoving = true;						//mouse is moving for drawing or moving the grid.
		if(!bKeySpaceEn)							//Not for zoom and move.
		{
			...
			else if(qPointCursor == Text)
			{
				...
				else
				{
					...
					bMouseMoving = false;			//when it is drag and drop operation, the mouse is to be not moving.
				}
			}
		}
	}
...
void QDrawArea::mouseMoveEvent(QMouseEvent *e)
{
	...
	if(bMouseMoving)
	{
		if(!bKeySpaceEn)		//for drawing graph
		{
			if(qPointCursor == Point
				||qPointCursor == Brush
				||qPointCursor == Erase)
